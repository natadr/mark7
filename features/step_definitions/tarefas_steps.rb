...
Dado("que o nome da minha tarefa é {string}") do |nome_tarefa|
  ...
  Quando("faço o cadastro desta tarefa") do
    @tarefas_page.cadastrar(@nome_tarefa, @data_finalizacao)
  end
  ...
  Então("devo ver a seguinte mensagem {string}") do |mensagem_alerta|
    expect(@tarefas_page.mensagem_aviso).to eql mensagem_alerta
  end  
  ...
  Então("devo ver somente {int} tarefa com o nome cadastrado") do |quantidade|
    ...
    @tarefas_page.voltar
    @tarefas_page.pesquisa_tarefa(@nome_tarefa)    
    expect(@tarefas_page.buscar_trs.size).to eql quantidade
  end
...
Então("esta tarefa não deve ser exibida") do
  @tarefas_page.pesquisa_tarefa(@nome_tarefa)    
    expect(@tarefas_page.buscar_trs.size).to eql 0
end
...
Então("esta tarefa deve permanecer na lista") do
  @tarefas_page.pesquisa_tarefa(@nome_tarefa)    
  expect(@tarefas_page.buscar_trs.size).to eql 1
end

# ALTERAR TAREFAS "TO DO"